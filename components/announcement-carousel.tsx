...
export default function AnnouncementCarousel({ images }: AnnouncementCarouselProps) {
  ...
  }, [isAutoPlaying, images.length]) // images.length bağımlılığı eklendi

  const nextSlide = () => {
    setCurrentIndex((prev) => (prev + 1) % images.length)
    setIsAutoPlaying(false)
    setTimeout(() => setIsAutoPlaying(true), 30000) // 30 saniye sonra otomatik geçişi tekrar başlat
  }
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Navigation Arrows */}
      {images.length > 1 && ( // Sadece birden fazla görsel varsa okları göster
        <>
          <Button
            variant="ghost"
            size="icon"
      ...
      {/* Dots Indicator */}
      {images.length > 1 && ( // Sadece birden fazla görsel varsa noktaları göster
